...
/**
 Retrieved this code from https://github.com/Chris911/iStats/
 */
...
#if 0
	char nameOfEntry[128];
/// Begin Testing Code
	io_registry_entry_t entries = IORegistryGetRootEntry(masterPort);
	if (entries == MACH_PORT_NULL)
	{
		printf("Error: IORegistryGetRootEntry() = %08x\n", result);
		return 1;
	}

	IORegistryEntryGetName(entries, nameOfEntry);
	printf("+%s\n", nameOfEntry);
	IORegistryIteratorEnterEntry(entries);
	

	entries = IOIteratorNext(entries);
	IORegistryEntryGetName(entries, nameOfEntry);
	printf("+%s\n", nameOfEntry);

	while (IOIteratorIsValid(entries))
	{
		io_object_t root = IOIteratorNext(entries);
	}
...
//	CFDataRef xml = CFPropertyListCreateXMLData(kCFAllocatorDefault, (CFPropertyListRef)Dict);
	CFErrorRef err;
	CFDataRef xml = CFPropertyListCreateData(kCFAllocatorDefault, (CFPropertyListRef)Dict,
